...
Then(
  `я вижу карточку категории, у которой внутри есть текст {string}`,
  (card: string) => {
...
When('ввожу в поле "Название" текст {string}', (text: string) => {
  I.fillField("input#category", text);
});
...
When(
  `не вижу карточку категории, у которой внутри есть текст {string}`,
  (card: string) => {
