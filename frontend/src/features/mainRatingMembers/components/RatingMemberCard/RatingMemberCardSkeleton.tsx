import { Skeleton } from '@/components/ui/skeleton';

const RatingMemberCardSkeleton = () => {
  return (
    <div className='flex flex-col items-center w-[33.2%] sm:w-[25%] xl:w-[20%]'>
      <Skeleton
        className='
          rounded-full mb-2 sm:mb-4 lg:mb-5
          w-[90px] h-[90px]
          xs:w-28 xs:h-28
          sm:w-36 sm:h-36
          lg:w-44 lg:h-44
          xl:w-48 xl:h-48
        '
      />
      <div className='flex flex-col items-center'>
        <Skeleton className='h-8 lg:h-10 w-12 sm:w-16 lg:w-24 mb-2 sm:mb-4 rounded-md xl:rounded-lg' />
        <Skeleton className='h-4 lg:h-5 w-12 sm:w-14 lg:w-20 rounded-sm xl:rounded-md' />
      </div>
    </div>
  );
};

export default RatingMemberCardSkeleton;
