...
export const NewsImagesModal: React.FC<Props> = ({ images, open, onClose, initialIndex = 0 }) => {
  ...
  return open ? (
    <div className='fixed inset-0 z-40 flex w-full items-center justify-center bg-black bg-opacity-50'>
      <Button
        onClick={handleClose}
        className='absolute z-30 top-3 right-3 bg-transparent text-white'
        variant='outline'
        size='icon'
      >
        <XIcon />
      ...
      <Carousel onClick={(e) => e.stopPropagation()} setApi={handleSetApi}>
        ...
        <CarouselPrevious className='absolute top-1/2 left-10 h-12 w-12 bg-transparent text-white z-10' />
        <CarouselNext className='absolute top-1/2 right-10 h-12 w-12 bg-transparent text-white z-10' />
      </Carousel>
    </div>
  ) : null;
};
